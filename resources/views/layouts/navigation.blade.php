{{-- Latar belakang diubah menjadi slate-900/80 dengan backdrop-blur, border diubah ke slate-700 --}}
<nav x-data="{ open: false }" class="bg-slate-900/80 backdrop-blur-sm border-b border-slate-700">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @elseif(Auth::user()->role === 'pengguna' && !Auth::user()->isMember())
                        <x-nav-link :href="route('membership.index')" :active="request()->routeIs('membership.index')">
                            {{-- Aksen warna diubah ke cyan --}}
                            <span class="text-cyan-400 font-bold">{{ __('Upgrade Membership') }}</span>
                        </x-nav-link>
                    @endif
                </div>
            </div>
            ...
            <div class="-me-2 flex items-center sm:hidden">
                <button @click="open = ! open" class="inline-flex items-center justify-center p-2 rounded-md text-slate-500 hover:text-slate-400 hover:bg-slate-900 focus:outline-none focus:bg-slate-900 focus:text-slate-400 transition duration-150 ease-in-out">
